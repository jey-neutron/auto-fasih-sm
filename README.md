...
Disini kita menggunakan Playwright di versi 2.0.0 untuk mengautomasi browser Chrome anda, pake beberapa request Get & Post. Download aja folder semua ini dan jalankan file [`.exe`](/Auto_Fasih_SM.exe)nya. 


# Step-by-step 🚗
> **👼Tutorial versi "Bahasa Bayi" [`disini`](/README-TUTOR-AUTO-FASIH.pdf)**

Ketika file [`Auto_Fasih_SM.exe`](/Auto_Fasih_SM.exe) udah jalan, maka muncul window aplikasi baru:
...
4. Jika ingin run auto-approve, ada beberapa parameter yang perlu dimasukkan:
	...
	- `"Fungsi Tambahan"`, ini modul yang dapat anda modifikasi untuk mendapatkan data di Fasih-SM, atau web lain, contoh yang ada di sini adalah `getdataPES` (untuk mendapatkan isian Fasih PES).
		...
		> Anda bisa menambahkan function sendiri di [`get_data.py`](/get_data.py) kemudian memasukkan nama functionnya itu di field ini
...
# Fitur📱
Fitur yang sudah tersedia di [`get_data.py`](/get_data.py) untuk saat ini:
...
- **getdataReview** (function untuk mengambil data review aja, tanpa get isian dalemnya, sama kayak get_list_data tapi return/mengembalikan data by id)
